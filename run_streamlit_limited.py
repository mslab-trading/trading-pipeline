...
# streamlit run run_streamlit.py --server.port 7777
...
def get_result_dir(model, category, strategy):
    return os.path.join("results_backtest_streamlit", model, category, strategy)

def get_returns(model, category, strategy, start_date: datetime, end_date: datetime):
    result_dir = get_result_dir(model, category, strategy)
    returns = pd.read_csv(f"{result_dir}/returns.csv")
...
def get_portfolio_value(model, category, strategy, start_date: datetime, end_date: datetime):
    result_dir = get_result_dir(model, category, strategy)
    portfolio_value = pd.read_csv(f"{result_dir}/portfolio_value.csv")
...
def get_signals(model, category, strategy):
    result_dir = get_result_dir(model, category, strategy)
    buy_signals_path = f"{result_dir}/buy_signals.csv"
    try:
        ...
        buy_signals.fillna(0. if strategy == "daily" else False, inplace=True)
    except FileNotFoundError:
        buy_signals = None

    sell_signals_path = f"{result_dir}/sell_signals.csv"
    try:
        ...
        sell_signals.fillna(0. if strategy == "daily" else False, inplace=True)
    except FileNotFoundError:
        sell_signals = None
...
def output_planned_trades(tab, strategy: str, buy_signals: pd.DataFrame, sell_signals: pd.DataFrame | None):
    ...
    if strategy == "allen":
        # Allen
        tab.text("購買股票(開盤)：model prediction > PR75 且 ADX > 40")
        tab.text("賣出股票(開盤)：model prediction < PR25")
    ...
    elif strategy == "gino" or strategy == "gino_open":
        # Gino:
        if strategy == "gino":
            tab.text("購買股票(收盤)：model prediction Top1")
            tab.text("賣出股票(收盤)：放滿 30 天的股票")
        elif strategy == "gino_open":
            tab.text("購買股票(開盤)：model prediction Top1")
            tab.text("賣出股票(開盤)：放滿 30 天的股票")
...
# main
st.write("Backtest Dashboard (AI Trading)")

all_models = ["StockAttentioner"]
model_format_mapping = {
    "StockAttentioner": "StockAttentioner (trading-pipeline)",
}
all_categories = ["Top50_RAM"]
all_strategies = ["allen", "gino", "gino_open", "daily"]
min_value, max_value = [datetime.date(2021, 1, 1), datetime.date.today()]
default_date_range = (datetime.date(2025, 6, 1), max_value)

with st.container(border=True):
    model = all_models[0]
    category = all_categories[0]
    strategy = strategy = st.selectbox("Strategy", all_strategies)
...
# Tab 1
returns = get_returns(model, category, strategy, start_date, end_date)
...
# Tab 2
portfolio_value = get_portfolio_value(model, category, strategy, start_date, end_date)
...
# Tab 3
signals = get_signals(model, category, strategy)
